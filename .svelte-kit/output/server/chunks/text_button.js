...
/* empty css        */const Text_button = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<button class="${escape(buttonClass, true) + " transition-all m-0 hover:m-0 rounded-full border-2 hover:border-2 " + escape(onPrimary ? "border-onPrimary" : "border-onBackground", true) + " " + escape(
    ...
    true
  )}" style="${"transition-duration: 100ms; transition-timing-function: cubic-bezier(.2, 0, .4, 1)"}"><b2>${slots.default ? slots.default({}) : ``}</b2></button>`;
});
export {
  Text_button as T
};

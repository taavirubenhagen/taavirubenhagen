...
<main class="font-sans"><section class="w-screen h-screen background p-8 md:p-16"><t2 class="mb-24">Shop
    </t2>
    ...
    return `<div class="transition duration-200 relative w-64"><img src="${"https://api.lorem.space/image/" + escape(["watch", "furniture", "fashion"][i % 3], true) + "?w=150&h=150"}" alt="Shop Item" class="rounded-lg w-full primary">
                <div class="absolute z-10 bottom-0 backdrop-brightness-75 backdrop-blur-sm rounded-b-lg w-full p-4 flex flex-col items-end"><div class="w-full flex flex-col"><t5 class="text-white">Inkless Pen</t5>
                        <t5 class="text-white">15$</t5></div>
                    </div>
            </div>`;
  })}</div></section></main>`;
});

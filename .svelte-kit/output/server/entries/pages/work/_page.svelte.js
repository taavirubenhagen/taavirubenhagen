    ...
    <div class="${"grid grid-cols-1 md:grid-cols-2 auto-rows-fr gap-8"}">${each(
          ...
          (data, i) => {
            return `${validate_component(Interactive, "Interactive").$$render($$result, { state: "hover" }, {}, {
              default: () => {
                return `<a${add_attribute("href", data[2], 0)}><button class="${"w-full h-full"}"><div class="${"default_animation_long hover:invert rounded-2xl border border-onPrimary w-full h-full bg-primary p-8 text-justify"}"><h4 class="${""}">${escape(data[0])}</h4>
                <div class="${"h-8"}"></div>
